...
final class TerminalCommandLauncher {
    ...
    func runViaAutomation(_ command: String) -> String? {
        ...
        if code == -1743 {
            return "Terminal automation is turned off for Limit Lifeboat. Enable it in System Settings → Privacy & Security → Automation to log in with one click."
        }
    ...
    func runViaCommandFile(_ command: String) -> Bool {
        let scriptURL = fileManager.temporaryDirectory
            .appendingPathComponent("limit-lifeboat-login-\(UUID().uuidString).command")
