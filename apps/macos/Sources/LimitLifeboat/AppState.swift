...
final class AppState: ObservableObject {
    ...
    private func refreshClaudeUsage() async {
        ...
        for profile in claudeProfiles {
            let priorState = refreshStates[profile.id] ?? .idle
            ...
            do {
                ...
                await enrichAccountInfoIfMissing(for: profile, accountIsLiveElsewhere: liveElsewhere)
            } catch {
                ...
                if case .rotationDeferred = fetchError {
                    // Inactive account whose token expired: a background cycle
                    // deliberately doesn't rotate it (rotation is reserved for
                    // switch/Retry to avoid churning the shared refresh chain).
                    // Keep the last snapshot — no error, no event. Preserve a
                    // real, sticky problem from an earlier user action (a dead
                    // login or locked keychain), but reset a transient prior
                    // error to idle: background cycles won't re-evaluate this
                    // account, so a one-off network blip must not freeze the row.
                    AppLog.usage.debug("Deferred background token rotation for inactive account \(profile.id, privacy: .public)")
                    switch priorState {
                    case .needsLogin, .keychainLocked, .ok:
                        refreshStates[profile.id] = priorState
                    default:
                        refreshStates[profile.id] = .idle
                    }
                    clearUsagePaused(for: profile.id)
                    continue
                }
    ...
    private static func credentialOutcome(for error: ClaudeAccountUsageFetchError) -> AppEvent.CredentialOutcome? {
        ...
        // A locked/denied keychain is an access problem, not a credential
        // outcome — it's surfaced via the .keychainLocked row state and its own
        // .error log, so it isn't recorded as a refresh event here.
        case .keychainLocked, .noCredentials, .transport, .rotationDeferred:
            return nil
        }
    }
    ...
    private func enrichAccountInfoIfMissing(for profile: AccountProfile, accountIsLiveElsewhere: Bool) async {
    ...
    private func preflightSwitchTarget(_ profile: AccountProfile) async -> SwitchPreflightResult {
        ...
        case .claude:
            do {
                // A switch is validating and activating this target, so it may
                // rotate an expired token even on an unattended auto-switch
                // (which runs non-interactively). Not a shared-account sibling:
                // switching to it is a deliberate choice to make it the live
                // login, so its own chain is the one being activated.
                let snapshot = try await claudeUsageService.fetchSnapshot(
                    for: profile,
                    isActiveCLI: profile.isActiveCLI,
                    permitRotation: true,
