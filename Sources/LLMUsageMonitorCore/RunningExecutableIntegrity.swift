...
public enum RunningExecutableIntegrityError: Error, LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .unavailable(let path):
            return "The running app executable no longer exists at \(path). Quit and relaunch LLM Usage Monitor before accessing credentials."
        case .replaced(let path):
            return "The running app executable was replaced at \(path). Quit and relaunch LLM Usage Monitor before accessing credentials."
        }
    }
}
