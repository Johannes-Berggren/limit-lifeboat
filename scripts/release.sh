...
TEAM_ID_SOURCE="$ROOT_DIR/Sources/LimitLifeboat/DistributionIdentity.swift"
DMG_BASENAME="Limit-Lifeboat-$RELEASE_VERSION-$ARCHITECTURE.dmg"
DMG_PATH="$ROOT_DIR/dist/$DMG_BASENAME"
...
[[ "$PINNED_TEAM_ID" =~ ^[A-Z0-9]{10}$ ]] \
  || fail "Confirm the release team and replace UNCONFIGURED in $TEAM_ID_SOURCE before releasing"
...
for command_name in codesign ditto git hdiutil lipo mount plutil security sed shasum spctl swift xcrun; do
...
ORIGIN_MAIN="$(git -C "$ROOT_DIR" rev-parse --verify refs/remotes/origin/main^{commit} 2>/dev/null || true)"
[[ -n "$ORIGIN_MAIN" ]] \
  || fail "The fetched origin/main ref is unavailable; run git fetch origin before releasing"
[[ "$(git -C "$ROOT_DIR" rev-parse HEAD)" == "$ORIGIN_MAIN" ]] \
  || fail "HEAD must match the fetched origin/main commit exactly"
[[ "$(git -C "$ROOT_DIR" rev-parse HEAD)" == \
    "$(git -C "$ROOT_DIR" rev-parse --verify "$EXPECTED_TAG^{commit}" 2>/dev/null || true)" ]] \
